...
│               │   ├── summary view     — 5-stat row + per-staff + list
...
│               ├── VerticalSwitcher     — modal, all 3 verticals, gated when cart has items
...
- **`useVerticalSettings`** (`hooks/useVerticalSettings.ts`) — per-vertical settings overrides via Context, falls back to `lib/verticals.ts` defaults at runtime via `getDefaults(verticalId)`, persists per-vertical to `nuatis-pos:{verticalId}:settings`. Reloads when `activeVerticalId` changes. Exposes: `settings`, `updateBusiness`, `updateTaxRate`, `updateTipPresets`, `updateStaff`, `resetSection`, `resetAll`. All update callbacks are stable refs (no re-creation on vertical switch — uses `verticalIdRef` pattern).
...
- **Confirm Charge** → `useCheckout.confirmCheckout(data)` → `calcTaxWithRate(subtotalCents, settings.taxRatePercent)` → state `tip → processing` → setTimeout → state `processing → receipt`
...
- **Vertical switch** → `setActiveVerticalId(id)` → localStorage write → context update → `useCart` reloads from new namespace → `useVerticalSettings` reloads settings for new vertical → `RegisterPage` reloads held tickets → tile grid re-renders with new vertical's services + colors
...
- `calcTaxWithRate(subtotalCents, ratePercent)` = `Math.round(subtotalCents × (ratePercent / 100))` — **this is the canonical tax helper**; `ratePercent` comes from `settings.taxRatePercent` in `useVerticalSettings`, which defaults to `8.25` if no override is saved
...
**Key helpers** (`lib/storage.ts`): `cartKey(v)`, `transactionsKey(v)`, `heldTicketsKey(v)`, `settingsKey(v)`, `ACTIVE_VERTICAL_KEY`, `ACTIVE_STAFF_KEY`.
...
**Isolation guarantee**: switching verticals never merges or copies cart/transaction/held/settings data across namespaces. Each vertical starts fresh or resumes its own last state.
...
| File | Exports | Notes |
|---|---|---|
| `lib/verticals.ts` | `VerticalId`, `VerticalConfig`, `VERTICALS`, `getActiveVerticalConfig` | Central per-vertical config: services, modifiers, category colors, business identity defaults for all 3 verticals (salon, spa, nail_bar) |
...
| `lib/storage.ts` | `cartKey`, `transactionsKey`, `heldTicketsKey`, `settingsKey`, `ACTIVE_STAFF_KEY`, `ACTIVE_VERTICAL_KEY`, `runMigrations` | Storage key helpers + one-time migration |
...
| `lib/cartMath.ts` | `calcLineTotalCents`, `calcLineDiscountCents`, `calcSubtotal`, `calcTax`, `calcTaxWithRate`, `calcTip`, `calcTotal`, `TAX_RATE`, `MANAGER_DISCOUNT_THRESHOLD` | Pure functions, integer cents only; `calcTaxWithRate` is canonical at runtime; `TAX_RATE` / `calcTax` are fallback constants only |
...
- Per-vertical settings scoping pattern (one tenant = one vertical in production, but the override layer model still applies to "vertical defaults vs tenant edits")
- Settings defaults read from config at runtime — not baked at build time; `getDefaults(verticalId)` always reflects the current `VERTICALS` registry
...
- Engine generalisation pattern: `VERTICALS` registry + `getActiveVerticalConfig` — 4th vertical requires only registry addition + `VerticalId` type extension
...
- String-literal type guards on `VerticalId` (e.g. `saved === "salon" || saved === "spa" || saved === "nail_bar"` in `useActiveVertical.ts`) — production should use `Object.keys(VERTICALS).includes(saved)` with cast pattern, flagged in Batch 13 report
